...
#from muss.mining.preprocessing import yield_json_documents_from_compressed, sentence_tokenize_document
...
def get_shard_path(sentences_path):
    shard = str(sentences_path)[-11:-7]
    dataset_dir = get_dataset_dir('uts') / 'pt'
    raw_original_dir = dataset_dir / 'raw_original'
    return raw_original_dir / f'pt_head_{shard}' / '000.json.gz'
...
def get_nearest_sentence_ids(query_index, db_index, topk, nprobe, batch_size=1024, use_gpu=True):
    try:
        ...
        if 'could not set parameter nprobe' in str(e):
            ...
            raise e
    if use_gpu:
        db_index = faiss.index_cpu_to_all_gpus(db_index, ngpu=2)
...
def get_context_by_shard(shard_path, sentence):
    for json_document in tqdm(yield_json_documents_from_compressed(shard_path), desc='documents'):
        sentences = sentence_tokenize_document(json_document.pop('raw_content'), language='pt')
        #document = json_document.pop('raw_content').replace("\n"," ").replace('\x00', ' ').replace('\t', ' ')
        #document = normalize_punctuation(normalize_unicode(document))
        #sentences = to_sentences(document, language='pt')
        if sentence not in sentences:
            continue
        for candidate in sentences:
            if not is_contained(candidate, sentence):
                return candidate
...
def get_db_context_sentences(sentences, sentences_paths, sentence_ids):
    sorted_idx = np.argsort(sentence_ids)
    sentence_ids = sentence_ids[sorted_idx]
    sentences = np.array(sentences)
    sentences = sentences[sorted_idx]
    ids_per_file = defaultdict(list)
    n_sentences_list = [cached_count_lines(sentences_path) for sentences_path in sentences_paths]
    offsets = np.cumsum([0] + n_sentences_list[:-1])
    next_offsets = np.cumsum(n_sentences_list)
    sentence_ids = np.sort(sentence_ids)
    for offset, next_offset, sentences_path in zip(offsets, next_offsets, sentences_paths):
        selected_sentences = sentences[(offset <= sentence_ids) & (sentence_ids < next_offset)]
        if len(selected_sentences) > 0:
            #selected_sentence_ids -= offset
            ids_per_file[sentences_path].extend(selected_sentences.tolist())
    # The sentences should be returned in the correct order because python dicts are insertion ordered
    sentences_list = []
    for sentences_path, sentences in tqdm(ids_per_file.items(), desc='Load sentences'):
        shard_path = get_shard_path(sentences_path)
        sentences_list.append([get_context_by_shard(shard_path, sentence) for sentence in sentences])
        
    sentences = [sentence for sentences in sentences_list for sentence in sentences]
    # Put sentences back in order
    return [sentences[idx] for idx in np.argsort(sorted_idx)]

def get_query_context_sentences(sentences, sentences_paths):
    context_sentences = []
    if len(sentences_paths==1):
        shard_path = get_shard_path(sentences_paths[0])
        for sentence in sentences:
            context_sentences.append(get_context_by_shard(shard_path, sentence))
    else:
        print("to do")
...
def find_nearest_neighbors(
    ...
    #query_context_sentences = get_query_context_sentences(query_sentences, query_sentences_paths)
    db_sentences = get_sentences_from_ids(db_sentence_ids, db_sentences_paths)
    #db_context_sentences = get_db_context_sentences(db_sentences, db_sentences_paths, db_sentences_ids)
...
def compute_and_save_simplification_pairs(
    ...
    if not simplifications_path.exists():
        ...
        with log_action('filtering'):
            ...
            #pairs_with_context = get_pairs_context(simplification_pairs)
